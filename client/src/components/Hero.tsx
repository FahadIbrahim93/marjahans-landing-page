...
export function Hero() {
  ...
  useEffect(() => {
    if (titleRef.current) {
      ...
      setTimeout(() => {
        if (titleRef.current) {
          titleRef.current.style.transition = 'all 1s cubic-bezier(0.34, 1.56, 0.64, 1)';
    ...
    if (ctaRef.current) {
      ...
      setTimeout(() => {
        if (ctaRef.current) {
          ctaRef.current.style.transition = 'all 1s cubic-bezier(0.34, 1.56, 0.64, 1)';
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 text-center space-y-8 px-6 max-w-4xl">
        ...
        <h1
          ref={titleRef}
          className="text-fluid-hero font-bold text-white tracking-widest leading-tight"
        >
          MARJAHAN'S
        </h1>
        ...
        >
          Where <span className="text-amber-400 font-semibold">LUXURY</span> meets{' '}
        ...
        <p className="text-sm text-slate-400 tracking-widest uppercase max-w-xl mx-auto leading-relaxed">
          Discover our curated collection of exquisite jewelry, handcrafted with precision and passion
        </p>

        <div ref={ctaRef} className="pt-8">
          <Button
            size="lg"
            className="bg-amber-600 hover:bg-amber-500 text-white rounded-full px-12 py-6 uppercase tracking-widest font-bold transition-all duration-300 hover:shadow-lg hover:shadow-amber-500/20"
          >
            Explore Collection
          </Button>
        </div>
